...
start_peloton_valgrind_script = "screen -dm bash -l -c \"valgrind --tool=callgrind --trace-children=yes %s/peloton -D ./data\"" % (PELOTON_BIN)  # > /dev/null 2>&1 &"
start_peloton_script = "%s/peloton -D ~/peloton-profile/data > /dev/null 2>&1 &" % (PELOTON_BIN)
stop_peloton_script = "%s/pg_ctl -t 2 -D ~/peloton-profile/data stop" % (PELOTON_BIN)

config_filename = "peloton_ycsb_config.xml"
# start_ycsb_bench_script = "%s/oltpbenchmark -b ycsb -c " % (OLTP_HOME) + cwd + "/" + config_filename + " --histograms --create=true --load=true --execute=false -s 5 -o "
start_ycsb_bench_script = "%s/oltpbenchmark -b ycsb -c " % (OLTP_HOME) + cwd + "/" + config_filename + " --histograms --execute=true -s 5 -o "
...
def bootstrap_peloton():
    shell_cmd = "cd ~/peloton-profile/; bash bootstrap_peloton.sh " + scheme
    cmd = "ssh %s@%s -t 'bash -l -c \"%s\"'" % (PELOTON_USER, PELOTON_HOST, shell_cmd)
    call(cmd, shell=True)

def start_peloton_valgrind():
    call(start_peloton_valgrind_script, shell=True)
    time.sleep(2)
